...
macro_rules! def_fn {
    ($( $trait:ident $fn:ident $tch_fn:ident $fn_:ident $tch_fn_:ident $($restr:ident)?),* $(,)?) => {
        $(
            impl<S: Shape, D: Device, K: Kind $(+ $restr)?> Tensor<S, D, K> {
                pub fn $fn<T: IntoScalar>(&self, rhs: T) -> Tensor<S, D, K::To>
                where
                    K: $trait<T>,
                {
                    Tensor {
                ...
                pub fn $fn_<T: IntoScalar>(&mut self, rhs: T) -> Tensor<S, D, K::To>
                where
                    K: $trait<T> + Same<K::To>,
                {
                    Tensor {
